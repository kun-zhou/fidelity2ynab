...
class TransactionDeduplicator {
  ...
  datesWithinTolerance(date1Str, date2Str) {
    const diffMs = Math.abs(new Date(date1Str) - new Date(date2Str));
  ...
  async findTransactionsToImport(fidelityTransactions, ynabTransactions) {
    ...
    console.log(`Analysis: ${toImportWithSuggestions.length} to import, ${toUpdate.length} to update, ${matched.length} already matched, ${unmatchedYnabTxns.length} unmatched YNAB after ${warningCutoffStr}, ${failedTransactions.length} failed to parse`);
