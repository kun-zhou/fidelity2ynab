...
const html = {
  ...
  twoColumnContainer(fidelityHtml, ynabHtml, summaryHtml = '') {
    return `
      <div class="flex flex-col h-full w-full max-w-full overflow-hidden">
        <div class="flex-shrink-0 max-w-full">${summaryHtml}</div>
        <div class="match-container relative flex gap-4 flex-1 min-h-0 max-w-full overflow-hidden">
          <div id="fidelityColumn" class="column-scroll max-w-full">
            <div class="text-xs font-medium text-gray-500 mb-2 sticky top-0 bg-gray-50 py-1 z-10">Fidelity Transactions</div>
            ${fidelityHtml}
          </div>
  ...
  matchSummary(counts) {
    ...
    if (toMatch > 0) parts.push(`<strong>${toMatch}</strong> to match`);
    if (toSkip > 0) parts.push(`<span class="text-gray-500">${toSkip} skipped</span>`);
    if (beforeWatermark > 0) parts.push(`<span class="text-gray-400">${beforeWatermark} previously imported</span>`);

    const total = toCreate + toMatch;
    if (total === 0) {
      return `<div class="text-xs text-gray-500 mb-3 text-center">All transactions up to date</div>`;
    }
    return `
      <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-3 flex justify-between items-center">
        <div class="text-sm text-gray-700">${parts.join(' &bull; ')}</div>
