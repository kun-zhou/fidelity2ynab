  ...
  async makeRequest(endpoint, method = 'GET', body = null) {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorDetail = errorData.error?.detail || errorData.error?.message || response.statusText;
      } catch {
...
class TransactionDeduplicator {
  ...
  getEarliestFidelityDate(fidelityTransactions) {
    ...
    const dates = fidelityTransactions
      .map(txn => this.parseFidelityDate(txn.date))
  ...
  parseFidelityDate(dateStr) {
    if (!dateStr || typeof dateStr !== 'string') {
      console.error('parseFidelityDate: invalid input:', dateStr);
      return null;
    }

    const parts = dateStr.split('-');
    if (parts.length !== 3) {
      console.error('parseFidelityDate: invalid format:', dateStr);
      return null;
    }
    ...
    if (!month || isNaN(year) || isNaN(day)) {
      console.error('parseFidelityDate: invalid components:', { monthAbbr, day, year, original: dateStr });
      return null;
    }
  ...
  fidelityToYNAB(fidelityTxn, accountId) {
    ...
    if (fidelityTxn.amountValue === undefined || fidelityTxn.amountValue === null) {
      throw new Error('Transaction missing amountValue field');
    }

    const date = this.parseFidelityDate(fidelityTxn.date);
    if (!date) {
      throw new Error(`Invalid date format: ${fidelityTxn.date}`);
    }
    ...
    } else if (fidelityTxn.status === undefined || fidelityTxn.status === null || fidelityTxn.status === '') {
    ...
    return {
      ...
      date: date,
      amount: amount,
      payee_name: fidelityTxn.description,
  ...
  isMatch(fidelityTxn, ynabTxn) {
    ...
    // Compare date
    const fidelityDate = this.parseFidelityDate(fidelityTxn.date);
    if (!fidelityDate) return false;

    // Check if transaction is a transfer (has transfer_account_id set)
    const isTransfer = ynabTxn.transfer_account_id !== null && ynabTxn.transfer_account_id !== undefined;
  ...
  async findTransactionsToImport(fidelityTransactions, ynabTransactions) {
    ...
    const earliestFidelityDate = this.getEarliestFidelityDate(fidelityTransactions);
    ...
    for (const txn of fidelityTransactions) {
      ...
      if (txnDate === null) {
        console.error('Failed to parse date:', txn);
    ...
    console.log(`Processing ${validFidelityTxns.length} of ${fidelityTransactions.length} Fidelity transactions`);
    ...
    for (const fidelityTxn of validFidelityTxns) {
      ...
      if (matchingYnabTxns.length > 0) {
        ...
        const isProcessing = fidelityTxn.status &&
    ...
    const toImportWithSuggestions = toImport.map(fidelityTxn => {
      ...
      const suggestions = ynabTransactions.filter(ynabTxn => {
        return !usedYnabIds.has(ynabTxn.id) && ynabTxn.amount === fidelityAmount;
      });
    ...
    const unmatchedYnabTxns = ynabTransactions.filter(ynabTxn => {
      return !usedYnabIds.has(ynabTxn.id) &&
             ynabTxn.date >= warningCutoffStr;
    });

    console.log(`Analysis: ${toImportWithSuggestions.length} to import, ${toUpdate.length} to update, ${pending.length} pending, ${matched.length} already matched, ${unmatchedYnabTxns.length} unmatched YNAB after ${warningCutoffStr}, ${failedTransactions.length} failed to parse`);
