...
class YNABApi {
  ...
  async createScheduledTransaction(budgetId, scheduledTransaction) {
    return (await this.makeRequest(`/budgets/${budgetId}/scheduled_transactions`, 'POST', { scheduled_transaction: scheduledTransaction })).data;
  }

  async getScheduledTransactions(budgetId) {
    return (await this.makeRequest(`/budgets/${budgetId}/scheduled_transactions`)).data.scheduled_transactions;
  }
}
